...
if page == 'Exploration et visualisation des données' :
    ...
    hypothèses = st.selectbox('Selectionner une hypothèse',('Hypothèse_1', 'Hypothèse_2', 'Hypothèse_3','Hypothèse_4'))
    ...
    if hypothèses == 'Hypothèse_4':
        ...
        sns.countplot(df.job=='student', hue=df.deposit).set_xlabel('job= etudiant')
        st.pyplot(fig4_1)
        
        st.write('##### Proportion de retraité par rapport à deposit')
        fig4_2 = plt.figure()
        sns.countplot(df.job=='retired', hue=df.deposit)
        sns.countplot(df.job=='retired', hue=df.deposit).set_xlabel('job= retraité')
        st.pyplot(fig4_2)   
